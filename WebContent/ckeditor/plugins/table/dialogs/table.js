				...
				if ( data.info )
				{
					...
					if ( !this._.selectedElement )
					{
						var tbody = table.append( makeElement( 'tbody' ) ),
						    //annasuccar-- replace info.txtRows by 2
							//rows = parseInt( info.txtRows, 10 ) || 0,
						    rows = parseInt( 2, 10 ) || 0,
					...
					//annasuccar- set by default headers to 'row'
					...
					//annasuccar- Column Footer
					/*var footer = info.footer;
					if ( !table.$.tFoot && footer )
					{
						var tfoot = new CKEDITOR.dom.element( table.$.createTFoot() );
						var row = tfoot.append( makeElement( 'tr' ) );
						tbody = table.getElementsByTag( 'tbody' ).getItem( 0 );
						var theRow = tbody.getElementsByTag( 'tr' ).getItem( 0 );
						cols = theRow.getChildCount();						
						for ( var j = 0 ; j < cols ; j++ )
						{
							var cell = row.append( makeElement( 'td' ) );
							if ( !CKEDITOR.env.ie )
							...
							[
								{
									...
									children :
									[
									 //annasuccar--commented out below
										/*{
											type : 'text',
											id : 'txtRows',
											'default' : 3,
										...
										//annasuccar--add a checkbox to create a column footer
										/*{
											type : 'checkbox',
											id : 'footer',
											'default' : '',
											label : editor.lang.table.footer,
											setup : function( selectedTable )
											{
												// Check if the table contains <tfoot>.
												if ( selectedTable.$.tFoot !== null )
