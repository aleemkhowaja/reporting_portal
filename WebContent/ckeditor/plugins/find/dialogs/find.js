...
(function() {
	...
	function c(i) {
		return !(i.type == CKEDITOR.NODE_ELEMENT && i
				.isBlockBoundary(CKEDITOR.tools.extend( {},
	...
	var d = function() {
		...
		return {
			textNode : i.textNode,
			offset : i.offset,
			character : i.textNode ? i.textNode.getText().charAt(i.offset)
					: null,
	...
	var h = function(i, j) {
		...
		var t = /[.,"'?!;: \u0085\u00a0\u1680\u280e\u2028\u2029\u202f\u205f\u3000]/, u = function(
			...
			find : function(y, z, A, B, C, D) {
				...
				while (G !== null) {
					...
					if (F == r) {
						if (A) {
							var H = M.matchRange.getCursors(), I = H[H.length - 1], J = H[0], K = new l(
									o(J), true), L = new l(n(I), true);
		...
		return {
				...
				this.selectPage = CKEDITOR.tools.override(this.selectPage,
						function(C) {
							return function(D) {
								...
								G = D === 'find' ? 'txtFindFind'
										: 'txtFindReplace';
								H = D === 'find' ? 'txtFindWordChk'
										: 'txtReplaceWordChk';
			...
			onShow : function() {
				var B = this;
				v.searchRange = w();
				var y = B.getParentEditor().getSelection().getSelectedText(), z = j == 'find' ? 'txtFindFind'
						: 'txtFindReplace', A = B.getContentElement(j, z);
				A.setValue(y);
				A.select();
				B.selectPage(j);
				B[(j == 'find' && B._.editor.readOnly ? 'hide' : 'show') + 'Page']
						('replace');
			},
			onHide : function() {
				var y;
