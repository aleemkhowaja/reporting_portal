...
$.widget('ui.spinner', {
	...
	_create: function() {
		...
		if(input.css("display") !== "none") //PathSolutions: when its hidden should not create buttons
	...
	_createButtons: function(input) {
		...
		var self = this,
			...
			wrapper = self.wrapper = input.css({ width: (self.oWidth = (box ? input.width() : input.outerWidth())) - buttonWidth,
				...
				//PathSolutions Denisk wrapping the input and spinner buttons into Table for proper alignment
					.wrap('<table border="0" cellpadding="0" cellspacing="0"><tr><td></td></tr></table>')
							.parent().after('<td></td>').next()/*.find('span')*/,
			btnContainer = self.btnContainer = $(
//				'<div class="ui-spinner-buttons">' +
					'<div class="ui-spinner-up ui-spinner-button ui-state-default  spanUpCorners"><span class="ui-icon '+options.upIconClass+'">&nbsp;</span></div>' +
					'<div class="ui-spinner-down ui-spinner-button ui-state-default spanDownCorners"><span class="ui-icon '+options.downIconClass+'">&nbsp;</span></div>'/* +
				'</div>'*/),
			// object shortcuts
		...
		input.addClass("spinnerTextCorners");
		if (msie)
		{ // IE not detecting class for corners directly.
			
		  var isRtl=(document.dir == "rtl"); 
		  var brdrIECss = "5px 0px 0px 5px" ;
		  if(isRtl)
			   brdrIECss = "0px 5px 5px 0px" 
		  input.css("border-radius",brdrIECss);
		}
		...
		var theBtnHeight = 8;
		var theCntrHeight = 19;
		if (msie || webkit)
		{
			//theBtnHeight = 8;
			theCntrHeight = 20;
		}
	// Path Solution Denisk, changed to static height and Margins for proper positioning		
		//wrapper.append(btnContainer.css({ height: height, left: -buttonWidth-rightMargin,
			// use offset calculation to fix vertical position in Firefox
		//	top: (input.offset().top - wrapper.offset().top) + 'px' }));
		wrapper.append(btnContainer);
		
		buttons = self.buttons = /*btnContainer*/wrapper.find('.ui-spinner-button');
		
		//buttons.css({ width: buttonWidth - (box ? buttons.outerWidth() - buttons.width() : 0), height: height/2 - (box ? buttons.outerHeight() - buttons.height() : 0) });
		buttons.css({ width: buttonWidth - (box ? buttons.outerWidth() - buttons.width() : 0), height: theBtnHeight });
		upButton = buttons[0];
		...
		icons.css({ marginLeft: -1, marginTop:  -4 });
