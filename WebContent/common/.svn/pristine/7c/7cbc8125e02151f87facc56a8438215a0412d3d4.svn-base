 ...
 */
	...
	,	east: {
		...
		,	togglerContent_closed:	opts["closeToggleText"]	
		}
	,	west: {
	...
	var initHandles = function (panes) {
		...
		$.each(panes.split(","), function (i, pane) {
			...
			,	_state	= (s.isVisible ? "-open" : "-closed") // used for classNames
				// INIT RESIZER BAR
			,	$R		= $Rs[pane] = $("<div class=\"ui-state-default\" ></div>")
				// INIT TOGGLER BUTTON
			...
			if (!s.isVisible && o.slidable)
				$R.prop("title", o.sliderTip).css("cursor", o.sliderCursor);

			$R
				// if paneSelector is an ID, then create a matching ID for the resizer, eg: "#paneLeft" => "paneLeft-resizer"
				.prop("id", (o.paneSelector.substr(0,1)=="#" ? o.paneSelector.substr(1) + "-resizer" : ""))
				.data("parentLayout", Instance)
			...
			if ($T) {
				$T
					...
					.prop("id", (o.paneSelector.substr(0,1)=="#" ? o.paneSelector.substr(1) + "-toggler" : ""))
					.data("parentLayout", Instance)
				...
				{	// ui-layout-closed
					...
					else
					{
						...
						$calcField = $('<div style="display:none">' + o.togglerContent_closed + '</div>');
 						$('body').append($calcField);
 						width = $calcField.width()+2;
 						$calcField.remove();

					}
					//PathSolutions Libin to add the arrow icon when closed for east and west pane----
					if(_pane=="-west")
						$("<span></span>").addClass("content-closed ui-icon ui-icon-circle-arrow-e ui-corner-all").appendTo( $T )
					else if(_pane=="-east")
						$("<span></span>").addClass("content-closed ui-icon ui-icon-circle-arrow-w ui-corner-all").appendTo( $T )
					if(document.dir=="rtl")// To give padding for the icons to align it correctly
						//$T.find(".ui-icon").css("padding-right","3px"); // PathSolutions [Libin] removed "padding-right" and added it as a style class "ui-layout-pane-icon-align"
						$T.find(".ui-icon").addClass("ui-layout-pane-icon-align");
					// ---
					$("<span>"+ o.togglerContent_closed +"</span>")
						.data("layoutRole", "togglerContent")
						.data("layoutEdge", pane)
						.addClass("content content-closed")
						.addClass("ui-layout"+_pane+"-header-closed")// PathSolutions Libin to seperately give class name based on pane (east or west)
						.css("display","none")
						.css("right",right)
						.css("left",left)
						.css("width",width)
		...
		$.each("toggle,open,close,pin,toggle-slide,open-slide".split(","), function (i, action) {
			$.each(_c.borderPanes.split(","), function (ii, pane) {
				$("."+pre+action+"-"+pane).each(function(){
					...
					name = $(this).data("layoutName") || $(this).prop("layoutName");
	...
	*/
	var close = function (pane, force, noAnimation, skipCallback) {
		...
		if (s.isSliding) // pane is being closed, so UNBIND trigger events
			...
			//PathSolutions Libin to get Correct PIN
			if ($(''+_c[pane].pins).length)
			{
				if(pane=="west" || pane=="east")
					$(''+_c[pane].pins).removeClass('ui-icon-pin-s').addClass('ui-icon-pin-w');
			}
			}
	...
	var setAsClosed = function (pane) {
		...
		if ($T) {
			$T
				...
				.prop("title", o.togglerTip_closed) // may be blank
			;
		...
		function open_2 () {
			...
			if (s.isVisible) { // make sure pane was not closed or hidden before animation finished!
				...
				if (!s.isSliding) {// resize all panes adjacent to this one
					...
					{
						if(pane=="west" || pane=="east")
							$(''+_c[pane].pins).removeClass('ui-icon-pin-w').addClass('ui-icon-pin-s');
					}
					}
	...
	var setAsOpen = function (pane, skipCallback) {
		...
		if ($T) {
			$T
				...
				.prop("title", o.togglerTip_open) // may be blank
			;
	...
	*/
	var bindStopSlidingEvents = function (pane, enable) {
		...
		if (!enable)
			...
			$R.prop("title", enable ? o.togglerTip_open : ""); // use Toggler-tip, eg: "Close Pane"
		}
	...
	*/
		...
		$.each(panes.split(","), function (i, pane) {
			...
			if ($T) {
				...
				if (!(togLen > 0) || togLen == "100%" || togLen > paneLen) {
					togLen = paneLen;
					// added below condition to set the offset to 0 only in case of east/west sides
					// without this condition, the togglers of north/south will also be set to 0 and
					// will appear at the far left instead of middle of the screen
					if (dir != "horz")
						offset = 0;
				}
	...
	var enableClosable = function (pane) {
		...
		$T	.bind("click."+ sID, function(evt){ evt.stopPropagation(); toggle(pane); })
			...
			.css("cursor", "pointer")
			.prop("title", state[pane].isClosed ? o.togglerTip_closed : o.togglerTip_open) // may be blank
			.show()
		;
	};
 ...
 */
	...
	*/
	function setPinState ($Pin, pane, doPin) {
		...
		;
		$Pin
			.prop("pin", doPin ? "down" : "up") // logic
			.prop("title", doPin ? lang.Unpin : lang.Pin)
