	...
	ajax: function(uri, options) {
		...
		if (options.method == 'post') {
			...
			// Path-Solutions - Modify the data type to fix the issue of parsing empty xml in IE
  			// request.dataType = ($.browser.msie) ? 'text' : 'xml' ;
  			// Always use text data type and parseXML of text reponse
		...
		// Path-Solutions - to fix the issue of Aborted HTTP Requests in IE8 that is reproduced
		// when the AlertAjaxServlet will respond slowly to initialization request. The fix is to 
		// use the connection = close in order to close the connection when the response is received. 
