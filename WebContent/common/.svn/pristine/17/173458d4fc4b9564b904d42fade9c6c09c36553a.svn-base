...
joint.dia.GraphCells = Backbone.Collection.extend({
    ...
    model: function(attrs, options) {
        ...
        if (attrs.type === 'link') {
        	...
        	//set the customdetails when creating the link the first time
        	var newLink = new joint.dia.Link(attrs, options);
        	if(attrs.customDetails != undefined && attrs.customDetails != null)
        	{
        		$(newLink).data('customDetails', attrs.customDetails);
        	}
            return newLink;
        }
        ...
        default:
            ...
            if (targetParentEvent) {
                ...
                if (targetParentEvent === 'remove') {
                	this.model.remove();
                	//Pathsolutions. when a link is delete we should fire an event to update the json value of the tree's input hidden
                	if(this.model.isLink())
                	{
                		this.paper.trigger("link-remove", evt, this, x, y);
                	}
                } else {
...
joint.dia.Paper = Backbone.View.extend({
    ...
    addCell: function(cell) {
        ...
        if(view.model != undefined && view.model != null
                && cell.attributes != undefined && cell.attributes != null)
        {
        	$(view.model).data('customDetails',cell.attributes.customDetails);
        }
    },
