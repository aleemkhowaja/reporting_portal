		...
		if( screenParamArr != undefined && screenParamArr != null && screenParamArr.length > 0 )
		{
			...
			if(lookupIds!= undefined && lookupIds != null && lookupIds.length > 0)
			{
				$.each(screenParamArr , function(index, val) { 
					...
					if(lookupIds.indexOf("lookuptxt_"+val)>-1 )
					{
			...
			if(radioIds!= undefined && radioIds != null )
			{
				$.each(screenParamArr , function(index, val) { 
					var rId = screenParamArr[index];
					if(radioIds[rId]!=null){
						var inputMode = 'text';
						screenParamMap[screenParamArr[index]] = {'inputValue':radioIds[screenParamArr[index]],'inputMode':inputMode};
					}
...
function screenGeneratorProp_onBeforTopic(gridId,rowObj)
{
	...
	{
		//get lookup ids for screen parameters
		$('input[id^="lookuptxt_"][name^="dynElem_"]').each(function () {
		...
		//get radio ids for screen parameters
				$('input[type="radio"][value="-1"]').each(function () {
					var radioId = this.id.substring(0,this.id.length-2);
					var checkedRadioId = $('input[type="radio"]:checked', '#dynScreen_'+dynScreenId+'_FormId').val();
