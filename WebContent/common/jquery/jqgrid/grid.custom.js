...
$.jgrid.extend({
		...
		return this.each(function(){
			...
			var triggerToolbar = function() {
				...
				$.each($t.p.colModel,function(i,n){
					...
					v = $("#" + $t.id + "_gs_"+$.jgrid.jqID(this.name), (this.frozen===true && $t.p.frozenColumns === true) ?  $t.grid.fhDiv : $t.grid.hDiv).val();
					if(v) {
						sdata[nm] = v;
						/**
			...
			var clearToolbar = function(trigger, clickReset){
				var sdata={}, v, j=0, nm;
				...
				$.each($t.p.colModel,function(i,n){
					...
					switch (this.stype) {
						case 'select' :
							var v1;
							$("#" + $t.id + "_gs_"+$.jgrid.jqID(this.name)+" option",(this.frozen===true && $t.p.frozenColumns === true) ?  $t.grid.fhDiv : $t.grid.hDiv).each(function (i){
							...
							break;
						case 'text':
							$("#" + $t.id + "_gs_"+$.jgrid.jqID(this.name),(this.frozen===true && $t.p.frozenColumns === true) ?  $t.grid.fhDiv : $t.grid.hDiv).val(v);
							if(v) {
								sdata[nm] = v;
								j++;
							} else {
				...
				// in case clickReset parameter not provided or provided as true, then need to click on reset
				clickReset = (typeof clickReset != 'boolean') ? true : clickReset;
				if(clickReset)
				{
			...
			$.each($t.p.colModel,function(i,n){
				...
				if(this.search){
						...
						if(surl) {
							...
							$.ajax($.extend({
								...
								success: function(res,status) {
									...
									$("select",self).attr({name:cm.index || cm.name, id: $t.id + "_gs_"+cm.name});
							...
							if (oSv) {	
								...
								$(elem).attr({name:cm.index || cm.name, id: $t.id + "_gs_"+cm.name});
								var so, sv, ov;
					...
					case 'text':
						...
						$(thd).append("<input type='text' style='width:95%;padding:0px;' name='"+(cm.index || cm.name)+"' id='" + $t.id + "_gs_"+cm.name+"' value='"+df+"'/>");
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			//or html === undefined added by Richie for OADM #BUG 736768
			if($firstHeaderRow.html() === null || $firstHeaderRow.html() === undefined) {
