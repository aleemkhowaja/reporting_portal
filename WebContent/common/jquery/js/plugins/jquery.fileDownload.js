...
$.extend({
    ...
    fileDownload: function (fileUrl, options) {
        ...
        //[PathSolutions] encrypt parameters in case available
		if(settings.data != undefined && settings.data != null && settings.data != '')
		{
			settings.data = returnEncryptedData(settings.data);
		}
        ...
        if (httpMethodUpper === "GET") {
            ...
            if (settings.data !== null) {
                ...
                $.each(settings.data.replace(/\+/g, ' ').split("&"), function () {
                    ...
                    if (!key) return;
                    //incase there is " inside the string we have to replace it by '
                    var value = decodeURIComponent(kvp[1] || '').replace(/"/g, '\'');
