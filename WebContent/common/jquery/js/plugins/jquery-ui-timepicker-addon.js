...
function Timepicker() {
	...
	this.regional[''] = { // Default regional settings
		...
		amNames: ['AM', 'A'],
		pmNames: ['PM', 'P'],
		timeFormat: 'hh:mm', //'hh:mm TT', //PathSolutions: changed default time to be without AM/PM
		timeSuffix: '',
...
$.extend(Timepicker.prototype, {
	...
	_newInst: function($input, o) {
		...
		//PathSolutions - #457149 Scheduler timing AM PM - set timeformat = 'hh:mm TT' in case timepicker is true and ampm is true and there isn't any timeformat passed from datepicker tag.
		if(o.timepicker == true && o.ampm == true
				&& (typeof o.timeFormat == 'undefined' || o.timeFormat == undefined || o.timeFormat == null || o.timeFormat == '' ))
		{
			o.timeFormat = 'hh:mm TT';
		}
		...
		tp_inst._defaults = $.extend({}, this._defaults, inlineSettings, o, {
			beforeShow: function(input, dp_inst) {
				if ($.isFunction(o.beforeShow))
					{ // PathSolutions adding the return to be able to make datepicker with time readonly 
			...
			onClose: function(dateText, dp_inst) {
				...
				if(tp_inst.$input.attr("fireChangeOnClose") === "true")
				{
					tp_inst.$input.attr("fireChangeOnClose","false");
	...
	_injectTimePicker: function() {
		...
		if ($dp.find("div#ui-timepicker-div-"+ dp_id).length === 0 && o.showTimepicker) {
			var noDisplay = ' style="display:none;"',
				html =	'<div class="ui-timepicker-div'+ (o.isRTL? ' ui-timepicker-rtl' : '') +'" id="ui-timepicker-div-' + dp_id + '"><dl>' +
	...
	_updateDateTime: function(dp_inst) {
		dp_inst = this.inst || dp_inst;
			...
			dt.setHours(dt.getHours() > 12 ? dt.getHours() + 2 : 0);//PathSolutions: added daylight saving adjustment
		...
		if(this.$input.attr("fireChangeOnClose") === "false")//change event fired 
		{
			this.$input.removeAttr("fireChangeOnClose");
		}
		else if($.trim(dp_inst.lastVal) !== $.trim(this.$input.val()))
		{
			this.$input.attr("fireChangeOnClose","true");
		}
		//this.$input.trigger("change");//PathSolutions: removed since it was firing 3 times, while .val fires the change event
	}
...
$.datepicker._doKeyPress = function(event) {
	...
	if (tp_inst) {
		if ($.datepicker._get(inst, 'constrainInput')) {
			...
			//
			var checkExist = event.ctrlKey || (chr < ' ' || !dateChars || datetimeChars.indexOf(chr) > -1);
			if(checkExist)
			{
...
$.datepicker._setDateDatepicker = function(target, date) {
	if(typeof date == "string" && date != null && date != "") //PathSolutions: in case of dependency date format is ISO string, time was not being set
	{
		var dateStr=date;
