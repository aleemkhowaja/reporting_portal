...
$.extend(CalendarsPicker.prototype, {
	...
	/**
	 * [pathSolution-MarwanMaddah] commands management to manage all links and buttons events 
	 */
	...
	/**
	 * [PathSolution-MarwanMaddah]: created the below Renderer to be compatible with UI-Themes
	 */
	themeRollerRenderer: {		
		picker: '<div class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all">' +
		'{months}' +
		...
		// dateFormat is optional and defaults to 'MM yyyy',
		// '{weekHeader}' to insert a week header, '{weeks}' to insert the month's weeks
		month: '<div class="ui-datepicker-group"><div class="ui-datepicker-header ui-widget-header ui-helper-clearfix ui-corner-all">{link:prev}{monthHeader}{link:next}</div>' +
		...
		// Extra class(es) for commands that are buttons
		commandButtonClass: 'ui-icon ui-icon-circle-triangle-w',
		// Extra class(es) for commands that are links
		commandLinkClass: '',
		// Class for disabled commands
		disabledClass: 'calendars-disabled'
	...
	_enablePlugin: function(target) {
		...
		if (inst.inline) {
			/**
			 * [PathSolution-MarwanMaddah]:change the href="fun" to href="#" onClick="func" 
			 */
			target.children('.' + this._disableClass).remove().end().
				find('button,select').removeAttr('disabled').end().
				find('a').attr('href', '#').end().find('a').attr('onclick','javascript:void(0)');
		}
		else {
	...
	_showPlugin: function(target) {
		...
		/**
		 * [pathSolutions-MarwanMaddah]: for the ReadOnly Management 
		 */
		if(typeof $(target).attr("readOnly")!= "undefined" && $(target).attr("readOnly")=="readonly")
		{
		  return;
		}
		/**
	 ...
	 * on laod will execute this function
	   @param  target  (event) a focus event or
	                   (element) the control to use
	...
	_update: function(target, hidden) {
		...
		if (inst) {
			...
			if (inst.inline) {
				...
				if (!inst.div) {
					/**
					 * [PathSolutions-MarwanMaddah]:added the id to the div to catch the hijri calendar by id when needed
					 */
					inst.div = $('<div id="ui-hijridatepicker-div"></div>').addClass(this._popupClass).
	...
	_updateInput: function(target, keyUp) {
		...
		if (inst) {
			 ...
			 * To set the Hijri date from the gregorian value onLoad 
			 */
			else
			{
			  this._setHijriOnload($(target).attr("id"));  	
			}
			/**
			 * 
			 */
	...
	_selectDatePlugin: function(target, elem) {
		...
		if (inst && !this._isDisabledPlugin(target)) {
			...
			/**
			 * [pathsolution-MarwanMaddah]: call set Gregorian date management
			 */
			var hijriDateId = $(target).attr("id");
			this._setGregorianDate(hijriDateId);
			/**
			 * 
			 */
	...
	/**
	 * [pathsolutions-marwanmaddah]: convert the selected value to Gregorian and set it on the related input
	 * when the date format changed, the split management should be changed
	 * @param {Object} value
	 * @param {Object} hijriDateId
	 */
    _setGregorianDate:function(hijriDateId)
    {
		    var value  = $("#"+hijriDateId).calendarsPicker('getDate');
			var dateSplit = value.toString().split("/");
			var gregorinaDate = this._islamicToGreg({HYear:dateSplit[0],HMonth:dateSplit[1],HDay:dateSplit[2]});
			var datePickerElemId = hijriDateId.substring(0,hijriDateId.indexOf("_hijriDate"))
			var datePickerElem = $("#"+datePickerElemId);
			datePickerElem.datepicker("setDate", gregorinaDate);
            datePickerElem.trigger("change");
    },
    
    /**
     * [PathSolution-MarwanMaddah]: Used to set the hijri date onLoad in case there is a gregorian value
     * @param {Object} hijriDateId
     */
    _setHijriOnload:function(hijriDateId)
    {
		var gregorianDate = $("#"+hijriDateId).attr("prevValue");
		if(typeof gregorianDate !="undefined" && gregorianDate!=null && gregorianDate!='')
		{
		    var timeSplitter    = gregorianDate.split("T");
		    var GregDateContent = timeSplitter[0].split("-");
		    var gregorianDate   = new Date(GregDateContent[0],GregDateContent[1]-1,GregDateContent[2]);
			var islamicCal      = $.calendars.instance('islamic');
			var hijriDate       = islamicCal.fromJSDate(gregorianDate);
			$("#"+hijriDateId).calendarsPicker('setDate', hijriDate);
		}
    },
/**
 * [PathSolution-MarwanMaddah: convert from hijri to gregorian]
 * @param {Object} floatNum
 * @return {TypeName} 
 */
_weekDay:function (wdn){
					if(wdn==0){
						return "Monday"
						}
					if(wdn==1){
...
/**
 * [pathSolution-MarwanMaddah]: used in the method that convert from islamic to Gregorian
 * @param {Object} floatNum
 * @return {TypeName} 
 */
_intPart:function (floatNum){
   ...
   /**
    * [pathsolutions-MarwanMaddah] : convert hijriDate to gregorian date
    * @param {Object} arg
    * @return {TypeName} 
    */
  _islamicToGreg:function (arg) {
				d=parseInt(arg.HDay)
				m=parseInt(arg.HMonth)
				y=parseInt(arg.HYear)
				delta=0
			//added 1 on jd to comply isna rulling
				jd=this._intPart((11*y+3)/30)+354*y+30*m-this._intPart((m-1)/2)+d+1948440-385-delta
				arg.JD=jd
				arg.wd =this._weekDay(jd%7)
								if (jd> 2299160 )
									{
									   l=jd+68569
									   n=this._intPart((4*l)/146097)
									   l=l-this._intPart((146097*n+3)/4)
									   i=this._intPart((4000*(l+1))/1461001)
									   l=l-this._intPart((1461*i)/4)+31
									   j=this._intPart((80*l)/2447)
									   d=l-this._intPart((2447*j)/80)
									   l=this._intPart(j/11)
									   m=j+2-12*l
									   y=100*(n-49)+i+l
									}
								else
									{
									 j=jd+1402
									 k=this._intPart((j-1)/1461)
									 l=j-1461*k
									 n=this._intPart((l-1)/365)-this._intPart(l/1461)
									 i=l-365*n+30
									 j=this._intPart((80*i)/2447)
									 d=i-this._intPart((2447*j)/80)
									 i=this._intPart(j/11)
									 m=j+2-12*i
									 y=4*k+n+i-4716
									}
				arg.CDay=d
				...
				var dateObj = dateToObjDate(d+"/"+m+"/"+y);
				return dateObj;
			},
	...
	_generateContent: function(target, inst) {
		...
		var addCommand = function(type, open, close, name, classes) {
			...
			/**
			 * [PathSolution-MarwanMaddah]:added to define the direction of Prev and Next
			 * added the span inside the link <a> for the triangle icon
			 * @memberOf {TypeName} 
			 */
			var isRTL   = inst.options.isRTL;
			var date = (inst.options.commandsAsDateFormat ? command.date.apply(target, [inst]) : null);
			var newName = isRTL?((name=='prev')?'next':'prev'):name; 
			picker = picker.replace(new RegExp('\\{' + type + ':' + name + '\\}', 'g'),
				'<' + open +
				(command.status ? ' title="' + inst.options[command.status] + '"' : '') +
				' class="' + inst.options.renderer.commandClass + ' ' +
				inst.options.renderer.commandClass + '-' + newName + ' ' + classes +
				(command.enabled(inst) ? '' : ' ' + inst.options.renderer.disabledClass) + '">' +
				'<span class="ui-icon ui-icon-circle-triangle-'+(name=='prev'?(isRTL?'e':'w'):(isRTL?'w':'e')) + '">'+
				'</span></' + close + '>');
		};
...
//				inst.options.renderer.commandLinkClass);
			addCommand('link', 'a href="#" onclick="javascript:void(0)"', 'a', name,
				inst.options.renderer.commandLinkClass);
			/**
			 * 
			 */
		...
		for (var week = 0; week < numWeeks; week++) {
			...
			for (var day = 0; day < calendar.daysInWeek(); day++) {
				...
				days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
					(selectable ? '<a href="#" onClick="javascript:void(0)"' : '<span') +
...
$.fn.calendarsPicker = function(options) {
	...
	/**
	 * [pathsolution MarwanMaddah]: set the compatible gregorian date on change
	 * @memberOf {TypeName} 
	 */
		if(options.inputId)
		{
			$("#"+options.inputId).bind("change",function()
			{
				var value = $("#"+options.inputId).val();
				if(value !=null && value !='')
			    {
					plugin._setGregorianDate(options.inputId)
			  	}
			});
		}
	/**
	 * end the part that added by pathsolutions
	 */
	return res;
};
