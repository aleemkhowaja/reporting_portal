...
<#include "/${parameters.templateDir}/path-xhtml/div-close.ftl" />
...
jQuery(document).ready(function () { 
	...
	options_${escapedOptionId?html}.onclick = "${parameters.onClickTopics?html}";
  	...
  	<#if parameters.onDBlClickTopics?exists>
	options_${escapedOptionId?html}.onDblClick = "${parameters.onDBlClickTopics?html}";
  	</#if>
  	<#if parameters.afterNodeCheckUncheckedTopic?exists>
	options_${escapedOptionId?html}.afterNodeCheckUncheckedTopic = "${parameters.afterNodeCheckUncheckedTopic?html}";
  	</#if>
	...
	options_${escapedOptionId?html}.disabled = ${parameters.disabled?string};
	</#if>
	<#if parameters.dndInSameTreeEnabled??>
	options_${escapedOptionId?html}.dndInSameTreeEnabled = ${parameters.dndInSameTreeEnabled?string};
	</#if>
	<#if parameters.dndCheckMoveAllowedFunc?if_exists != "">
	options_${escapedOptionId?html}.dndCheckMoveAllowedFunc = ${parameters.dndCheckMoveAllowedFunc?string};
	</#if>
	<#if parameters.dndMoveCompleteFunc?if_exists != "">
	options_${escapedOptionId?html}.dndMoveCompleteFunc = ${parameters.dndMoveCompleteFunc?string};
	</#if>
