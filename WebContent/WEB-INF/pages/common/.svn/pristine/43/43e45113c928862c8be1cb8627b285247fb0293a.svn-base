  	  ...
  	  <#if absoluteElements != 'true'> 
  	     ...
  	     <#assign key=formVO.element_list.get(index).listParamVO.key?default('')/>
  	        <#if positionAbsolute?default(false) == true>
  	           <div style = "${css_attr}">
		  	    <#if key==''>
	  		 		<@ps.radio considerChoiceValue="${considerChoiceValue}" dynValue = "${value}" list="${list}" name="${name}" onclick="${onclick}" id="${id}" disabled="${readOnly_att}" groupElemKeyLabel="${groupLabel}"/>
	  		 	<#else>
		  		    <#assign listValue=formVO.element_list.get(index).listParamVO.value/>
		  		    <#assign hasHiddenOpt=formVO.element_list.get(index).listParamVO.hasHiddenOpt/>
		  		    <@ps.radio considerChoiceValue="${considerChoiceValue}" dynValue = "${value}" list="${list}" name="${name}" onclick="${onclick}" id="${id}" disabled="${readOnly_att}" listKey="${key}" listValue="${listValue}" hasHiddenOpt="${hasHiddenOpt}" groupElemKeyLabel="${groupLabel}"/>
	  		    </#if>
  		       </div>	
  	        <#else>
		  	    <#if key==''>
	  		 		<@ps.radio considerChoiceValue="${considerChoiceValue}" dynValue = "${value}" list="${list}" name="${name}" onclick="${onclick}" id="${id}" disabled="${readOnly_att}"  cssStyle="${css_attr}" groupElemKeyLabel="${groupLabel}"/>
	  		 	<#else>
		  		    <#assign listValue   =formVO.element_list.get(index).listParamVO.value/>
		  		    <#assign hasHiddenOpt=formVO.element_list.get(index).listParamVO.hasHiddenOpt/>
		  		    <@ps.radio considerChoiceValue="${considerChoiceValue}" dynValue = "${value}" list="${list}" name="${name}" onclick="${onclick}" id="${id}" disabled="${readOnly_att}" listKey="${key}" listValue="${listValue}" hasHiddenOpt="${hasHiddenOpt}" cssStyle="${css_attr}" groupElemKeyLabel="${groupLabel}"/>
	  		    </#if>  	           
  	        </#if>
  	  <#if absoluteElements != 'true'>
  	  	</td>
